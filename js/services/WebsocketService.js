...
define(["app",
    ...
    if (runInBrowser()) {
        ...
        FLAGS = "Conversation, RemindMessage, noteRemind, pushChannelItem, pushTaskMsg, RetractDirective, readMessage";
    }
    ...
    function service($rootScope, UserService, $state, NativeService) {
        ...
        return {
            open: function () {
                ...
                if (socket == null) {
                    socket = websocket.openMsgPush(loginUser.userId, loginUser.email, loginUser.userName, loginUser.password, FLAGS, onMessage, onReconn, onFail, pkg.version, localIp, deviceId);
                    $rootScope.$bus.publish({
                        channel: 'websocket.event',
                        topic: 'connect',
                        data: {}
